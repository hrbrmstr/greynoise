#' The GreyNoise default font color
#' @docType data
#' @export
gn_text_col <- "#ccc9c8"
...
#' Change geom defaults from black to custom lights for the GreyNoise theme
#'
#' @export
gn_geom_defaults <- function() {
  ...
  for (g in geoms) {
    update_geom_defaults(gn, list(colour = "#617a89", fill = "#617a89"))
  }

  geoms <- c("text", "label")

  for (g in geoms) {
    update_geom_defaults(g, list(colour = gn_text_col, fill = "#252a32"))
  }
